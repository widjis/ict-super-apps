...
## WiFi hub visual follow-up — 2026-09-20 WIB

Scope: UI-only Phase 1 polish requested from the existing Asset Management screenshot. Reused the repository's surface/primary/tertiary/secondary tokens, typography, card radius/shadow and horizontal icon/title/description layout. No external template/dependency, backend, API, auth, OCR, photo, VPN or server changes. Check Status is one keyboard-operable whole-card button; Register Device and Lease Report are native disabled buttons with Coming soon labels and no chevrons. BottomNav and TopBar stay unchanged. The unavailable inventory disclaimer remains plain text instead of a fourth large card.

Verification:
- TDD: new whole-card/accessible-name/navigation/disabled-actions regression failed on old UI (`aria-labelledby` null), then passed. `npm test`: 48 frontend + 73 backend passed, no failures/skips; `/tmp/wifi-hub-tests.log`.
- `npm run lint` (TypeScript) and Vite build passed. `git diff --check` passed. Existing Node module-registration deprecation and Gradle/Capacitor warnings remain.
- Actual production components rendered via local isolated `test/fixtures/wifi-hub.html` + `wifi-hub.tsx`, no login tokens, secrets or real API requests needed. This is presentation/navigation evidence, not live integration evidence. Fixture has no production entry-point import and is absent from APK.
- Chromium widths 320/390/768: document width equals viewport, all three cards horizontal; sticky-header-to-eyebrow spacing 32px; at maximum scroll final explanatory text clears unchanged bottom navigation by at least 48.75px. Narrow viewport 320×740 and normal 390×844 visually inspected. Real Enter-key activation opens actual Check Status screen (`#wifi-mac`), visible keyboard outline confirmed. Local fixture uses existing font stack/fallback, not a claim of Android font/device parity.
- Screenshots: `/tmp/ict-wifi-hub-qa/wifi-hub-320.png`, `wifi-hub-390.png`, `wifi-hub-390-bottom.png`. Temporary artifacts may expire. A portion of the last card initially sits below the viewport, as in the reference; scrolling exposes the entire card and disclaimer above navigation.
- Clean Android rebuild with Android Studio Java 21.0.8 and existing SDK: `node scripts/run-gradle.mjs clean`, `npm run build:apkdebug` passed. Final `:app:testDebugUnitTest :app:lintDebug`: 24 native tests, zero failures/errors/skips; lint zero errors, 18 existing warnings. Logs `/tmp/wifi-hub-clean.log`, `/tmp/wifi-hub-apk.log`, `/tmp/wifi-hub-native.log`.
- APK `android/app/build/outputs/apk/debug/app-debug.apk`: **50,795,324 bytes**, SHA-256 **`c76fc70266b84e86ea035f2d012003b97c20ba0441a1493e4d5676715f7f4e50`**. ZIP verification: **11/11 dist files byte-identical**, local fixture absent, MacOcrPlugin retained in DEX. `/tmp/ict-wifi-hub-qa/artifact-verification.json`.
- Self-review: no added credentials, network/storage calls, unsafe HTML or backend mutations; native disabled semantics and explicit accessible names/descriptions preserved. Design audit: no new generic gradient, fake metrics, centered icon-topper or global restyling; exact local card language intentionally reused. Independent parent review/publication remains a delivery gate; subagent did not commit/push.

`docs/openapi.yaml` was absent in this checkout and remains unchanged: no endpoint/schema/auth/response change. `docs/implementation-roadmap.md` was absent and has been added as a scoped checkpoint, not a reconstructed global roadmap. Pre-existing dirty `AGENTS.md` is preserved and excluded from task publication. Prior backend-comment rollout remains blocked/not deployed; no retry. Physical-device APK/OCR/OEM validation remains separate and open.
